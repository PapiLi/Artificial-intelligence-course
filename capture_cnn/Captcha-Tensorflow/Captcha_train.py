...
from Captcha_data import number
...
def crack_captcha_cnn(w_alpha=0.01, b_alpha=0.1):
    ...
    w4 = tf.Variable(w_alpha * tf.random_normal([8 * 32 * 40, 1024]))
    ...
    w_o = tf.Variable(w_alpha * tf.random_normal([1024, MAX_CAPTCHA * CHAR_SET_LEN]))
    b_o = tf.Variable(b_alpha * tf.random_normal([MAX_CAPTCHA * CHAR_SET_LEN]))
        ...
        for step in range(2500):
           ...
           #print(step, loss_)
            if step % 100 == 0:
                ...
                if acc > 0.99:
                    model_path = os.getcwd() + os.sep + str(acc) + "captcha.model"
